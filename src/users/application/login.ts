...
export async function login(token: string | undefined, env: Bindings, db: Client): Promise<{body: any, status: 200 | 500 }> {
  if (token === undefined) {
    return {
      body: {
  ...
  if (retrievedUser.data.length === 0) {
    ...
    const token = await createJWT({
      id: retrievedUser.data[0].id,
      sub: retrievedUser.data[0].sub,
      is_admin: false
    },
    env)
    response = {
